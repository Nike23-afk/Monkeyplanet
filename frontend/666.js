...
function Devilish(){
    ...
    let audio = new Audio('./sfx/ems.mp3');
    audio.play();
    }, 1000)
}
function Hellish() {
    ...
    setInterval(function(){
                ...
                let elements = ["hell is upon us","august 12 2036 the <heat>heat death</heat> of the universe"];
                let randomElement = elements[Math.floor(Math.random()*elements.length)];
                let replacedText = document.querySelector('body').innerHTML.replaceAll(/./g, 'august 12 2036 the <heat>heat death</heat> of the universe');
                document.querySelector('body').innerHTML = replacedText.replace("august 12 2036 the <heat>heat death</heat> of the universe", `<font color='#${hexColor}'>${randomElement}</font>`);
    }, 500)
}
function s4() {
